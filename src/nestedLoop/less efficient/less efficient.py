...
while (True):
    ...
    # pricing
    # subtotal
    print("\nFor each item, large size will charge $2 more, small size will charge $2 less, medium size will remain the standard price.");
    ...
    # refunding
    print("\nHere is what you ordered:", orderCopy);
    userInput = input("Do you want to request any refunds? Enter \"true\" or \"false\": ");
    if (userInput == "true"):
        refund = input("Which thing do you want to return? Use dot (.) to separate each item (including the last one). Enter the items: ")
        for i in range(refund.count('.')):
            if ("beef burger." in refund):
                refund = refund.replace("beef burger", "", 1);
                subtotal -= 10;
                print("beef burger", end = "");
            elif ("cheese burger." in refund):
                refund = refund.replace("cheese burger", "", 1);
                subtotal -= 10;
                print("cheese burger", end = "");
            elif ("double burger." in refund):
                refund = refund.replace("double burger", "", 1);
                subtotal -= 15;
                print("double burger", end = "");
            elif ("chicken burger." in refund):
                refund = refund.replace("chicken burger", "", 1);
                subtotal -= 10;
                print("chicken burger", end = "");
            elif ("sausage burger." in refund):
                refund = refund.replace("sausage burger", "", 1);
                subtotal -= 10;
                print("sausage burger", end = "");
            elif ("fruitopia strawberry." in refund):
                refund = refund.replace("fruitopia strawberry", "", 1);
                subtotal -= 10;
                print("fruitopia strawberry", end = "");
            elif ("fruitopia orange." in refund):
                refund = refund.replace("fruitopia orange", "", 1);
                subtotal -= 10;
                print("fruitopia orange", end = "");
            elif ("diet coke." in refund):
                refund = refund.replace("diet coke", "", 1);
                subtotal -= 10;
                print("diet coke", end = "");
            elif ("coke." in refund):
                refund = refund.replace("coke", "", 1);
                subtotal -= 10;
                print("coke", end = "");
            elif ("fries." in refund):
                refund = refund.replace("fries", "", 1);
                subtotal -= 10;
                print("fries", end = "");
            elif ("ice cream." in refund):
                refund = refund.replace("ice cream", "", 1);
                subtotal -= 10;
                print("ice cream", end = "");
